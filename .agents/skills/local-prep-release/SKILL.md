...
- **Never modify files the user hasn't approved.** The only files this skill touches without asking are `build.gradle.kts` (version bump) and `CHANGELOG.md` (new section) — and only **after** the user has approved the draft release content.
...
1. **Working tree is clean.** Run `git status --porcelain`. It must be empty (untracked `.idea/` is OK — ignore it if it's the only thing).
   ...
   >
   > Typical flow: finish your feature, commit it with a message like `"Prep for release"` (or whatever describes the feature), then re-run `/local-prep-release`.
   >
...
5. **Relevant source files.** For any commit whose subject isn't self-explanatory, read the actual diff (`git show <sha>`) to understand what changed. Don't guess. The user is counting on you to produce a CHANGELOG entry they don't have to rewrite.
...
- **Omit internal-only changes.** Renames, refactors, test additions, ticket or skill edits — none of these belong in the user-facing changelog unless they have observable effects. If you're unsure, leave it out.
...
3. **Run `./gradlew --no-daemon test`.** Stream the output; pipe through `grep -E "(FAILED|BUILD|tests completed)" | head` to keep the context tight. If the build fails, **stop**. Print the failure and leave the working tree alone. Do not attempt to revert your edits; the user will inspect and decide.

4. **Run `./gradlew --no-daemon verifyPluginProjectConfiguration`.** Same discipline — stop on failure.
...
1. **Show the diff.** Run `git diff CHANGELOG.md build.gradle.kts` and show the output. The user will use this to review your work.
...
- **Release commits are clean**: they touch only `CHANGELOG.md` and `build.gradle.kts`. Feature work is *always* a separate preceding commit. If the user's working tree has mixed concerns, the pre-condition check in Phase 1 will catch it.
...
- **`verifyPluginProjectConfiguration`** is flagged in this project's `AGENTS.md` as the config-correctness check. Run it every time.
